...
public class SourceDetailActivity extends BaseActivity {
	...
	private Button mBack, mPlace, mAttention;

	private MyGridView mGridView;

	private ImageGridAdapter mAdapter;

	private TextView mOrderNumber, mName;
	...
	private void initViews() {
		...
		webview = (WebView) findViewById(R.id.source_webview);
	...
	private void showSourceDetail(NewSourceInfo sourceInfo) {
		...
		webview.loadDataWithBaseURL(null, sourceInfo.getCargo_remark(),
	...
	private void getSourceDetail(int order_id) {
		...
		try {
			params.put(Constants.ACTION, Constants.GET_SOURCE_ORDER_DETAIL2);
			params.put(Constants.TOKEN, application.getToken());
			params.put(Constants.JSON,
					new JSONObject().put("order_id", order_id).toString());
			showDefaultProgress();
	...
	private void attentionOrder() {
		final JSONObject params = new JSONObject();
		try {
			params.put(Constants.ACTION, Constants.ATTENTION_SOURCE_ORDER);
			params.put(Constants.TOKEN, application.getToken());
			params.put(Constants.JSON,
					new JSONObject().put("order_id", order_id).toString());
			showDefaultProgress();
