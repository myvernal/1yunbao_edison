	...
	public void onReceive(final Context context, Intent intent) {
		...
		if (JPushInterface.ACTION_MESSAGE_RECEIVED.equals(intent.getAction())) {
			...
			case 6:
				...
				// if (pushBean.getOrder_id() != 0) {
				// LogUtil.i(TAG, "包含订单号!!!");
				// update_loc_order_id = pushBean.getOrder_id() + "";
				//
				// } else {
				...
				LocHelper.getInstance(context).getResult(new LocCallback() {
					...
					public void onRecivedLoc(double lat, double lng, String addr) {
						...
						if (pushBean.getOrder_id() != 0) {
							ddId = pushBean.getOrder_id();
						} else if (pushBean.getId() != 0) {
							ddId = pushBean.getId();
						}

						LogUtil.i("MessageBroadCastReceiver", "获取到位置");

						MGApplication application = (MGApplication) context
